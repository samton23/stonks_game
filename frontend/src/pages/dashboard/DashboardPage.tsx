...
import { Crown, Factory, Building2, Trophy, Zap, QrCode, X } from 'lucide-react'
import { getDashboard, getRoomCode } from '../../api'
    ...
    <div className="min-h-screen bg-dark-900 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center mb-8">
          <motion.div animate={{ y: [0, -5, 0] }} transition={{ duration: 3, repeat: Infinity }} className="text-6xl mb-4">
            {gameFinished ? '🏆' : '📈'}
          </motion.div>
          ...
          {gameFinished ? (
            <motion.div animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 2, repeat: Infinity }} className="inline-flex items-center gap-2 mt-4 px-4 py-1.5 rounded-full bg-accent-gold/10 border border-accent-gold/20">
              <Trophy size={14} className="text-accent-gold" />
              <span className="text-xs text-accent-gold font-medium">ИГРА ЗАВЕРШЕНА</span>
            </motion.div>
          ) : (
            <motion.div animate={{ opacity: [1, 0.5, 1] }} transition={{ duration: 2, repeat: Infinity }} className="inline-flex items-center gap-2 mt-4 px-4 py-1.5 rounded-full bg-accent-green/10 border border-accent-green/20">
          ...
          <div className="mt-4">
            <button
              onClick={openQR}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors text-sm text-gray-400 hover:text-white"
            >
              <QrCode size={16} />
              Код комнаты
            </button>
